// ─── Mid buildings ─────────────────────────────────────────────────────────────
// Main building layer. Buildings with a `url` property are enterable — the
// proximity system in main.js checks their door screen position each frame.

// door: x offset of door within the building (px from building left edge)
// doorCenter: computed as building.x + door + 7 (half door width 14px)
//             used by the proximity system to find the door's world position.
// url: if set, the building is enterable and navigates to this path.

const midBuildingData = [
  { x: 20,   w: 70,  h: 90,  color: '#e8c98a', border: '#c9a55a', roof: true,  sign: null,
    windows: [{x:8,y:15},{x:8,y:35},{x:8,y:55},{x:40,y:15},{x:40,y:35},{x:40,y:55}],
    door: 28, url: null },
  ...
  { x: 630,  w: 110, h: 120, color: '#d8e0c8', border: '#8aa060', roof: true,
    ...
    windows: [{x:10,y:18},{x:10,y:45},{x:10,y:72},{x:65,y:18},{x:65,y:45},{x:65,y:72}],
    door: 48, url: 'contact.html' },

  { x: 790,  w: 80,  h: 90,  color: '#e8c8e8', border: '#a060a0', roof: true,  sign: null,
    windows: [{x:8,y:15},{x:8,y:40},{x:8,y:65},{x:48,y:15},{x:48,y:40},{x:48,y:65}],
    door: 33, url: null },
...
// ─── Enterable buildings ───────────────────────────────────────────────────────
// Derived from midBuildingData — only buildings with a url get proximity checks.
// doorWorldX is the centre of the door in world space (within the first tile).
const enterableBuildings = midBuildingData
  .filter(d => d.url && d.door !== null)
  .map(d => ({
    label:      d.sign ? d.sign.text : 'ENTER',
    url:        d.url,
    doorWorldX: d.x + d.door + 7,  // 7 = half of 14px door width
  }));
...
function createMidBuilding(d, ox, signFontSize) {
  ...
  el.className = 'mid-b';
  el.style.cssText = `
    left: ${d.x + ox}px; width: ${d.w}px; height: ${d.h}px;
    background: ${d.color}; border-color: ${d.border};
  `;
